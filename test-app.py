...
#%%
new_model.load_weights('weights/model')
...
while(True):
    ...
    if(classe=='Thumb Up' and  np.amax(predict)*100 > 95 and status == "closed"):
        print("Thumb Up oldu resim yüklenecek")
        status = "opened"
        cv2.imshow('image',img)

    if(classe=='Shaking Hand' and  np.amax(predict)*100 > 95 and status=='opened'):
        status = "closed"
        cv2.destroyWindow('image')

    if(classe=='Stop Sing' and  np.amax(predict)*100 > 98):
        cap.release()
